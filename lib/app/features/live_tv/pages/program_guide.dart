...
class ProgramGuide extends StatefulWidget {
  const ProgramGuide({
    super.key,
    required this.mduController,
    required this.onChannelMenuSelected,
  });

  final Mdu1Controller mduController;
...
class _ProgramGuideState extends State<ProgramGuide> {
  ...
  Future<void> initVideoControlCubit() async {
    final videoTracks =
        await widget.mduController.getTracks(TrackType.video) as List;
    final audioTracks =
        await widget.mduController.getTracks(TrackType.audio) as List;
    final captionTracks =
        await widget.mduController.getTracks(TrackType.captions) as List;

    if (!mounted) return;
    context.read<VideoControlCubit>().initValues(
          captionTracks.indexWhere((element) => element['isSelected'] == true),
          videoTracks.indexWhere((element) => element['isSelected'] == true),
          audioTracks.indexWhere((element) => element['isSelected'] == true),
          ...
          if (event is RawKeyDownEvent && isInfoSheetOpen) {
            ...
            switch (event.logicalKey.keyLabel) {
              ...
              case keyEnter:
                ...
                switch (popupItemType!) {
                  case PopupItemType.video:
                    final videoTracks = await widget.mduController
                        .getTracks(TrackType.video) as List;

                    await widget.mduController.selectTrack(
                      TrackType.video,
                      videoTracks[state.videoIdx]['trackGroupIndex'] as int,
                      videoTracks[state.videoIdx]['trackIndex'] as int,
                    );

                    break;
                  case PopupItemType.audio:
                    final audioTracks = await widget.mduController
                        .getTracks(TrackType.audio) as List;

                    await widget.mduController.selectTrack(
                      TrackType.audio,
                      audioTracks[state.audioIdx]['trackGroupIndex'] as int,
                      audioTracks[state.audioIdx]['trackIndex'] as int,
                    );
                    break;
                  case PopupItemType.captions:
                    final captionTracks = await widget.mduController
                        .getTracks(TrackType.captions) as List;

                    final trackGroupIndex = captionTracks[state.captionIdx]
                        ['trackGroupIndex'] as int;
                    final trackIndex =
                        captionTracks[state.captionIdx]['trackIndex'] as int;
                    final isOffSelected =
                        trackGroupIndex == -1 && trackIndex == -1;
                    DeviceId.ccEnabled = !isOffSelected;
                    ...
                    unawaited(
                      GetIt.I<ChopperClient>()
                          ...
                          .updateCCFlag(
                            closedCaptions: isOffSelected ? '0' : '1',
                          ),
                    );
                    break;
                }
                              ...
                              children: [
                                ...
                                buildButton(
                                  ...
                                  () async {
                                    ...
                                    final list = await widget.mduController
                                        .getTracks(TrackType.captions) as List;
                                    await SmartDialog.show(
                                                  ...
                                                  child: RadioListTile<dynamic>(
                                                    ...
                                                    onChanged: (selected) {
                                                      if (selected != null) {
                                                        ...
                                                        widget.mduController
                                                            .selectTrack(
                                                          TrackType.captions,
                                                          item['trackGroupIndex']
                                                              as int,
                                                          item['trackIndex']
                                                              as int,
                                                        );
